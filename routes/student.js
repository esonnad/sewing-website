  ...
  if(req.file) {
    ...
    .then(post => {
      transporter.sendMail({
        ...
        text: content,
        html: `<h1>The requested post:</h1><hr><h3>${header}</h3><p>${content}</p><p><i>by: ${creatorName}</i></p><br><hr><br><b><a href="https://naehspass.herokuapp.com/admin/confirmPost/${post._id}">Click here to accept and add the post</a></b>`
      })
        .then(info => res.render('student/successPost'))
        .catch(error => console.log(error));
    })
